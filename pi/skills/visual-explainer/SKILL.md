...
## Project Analysis Discipline

When explaining an existing project, architecture, diff, plan, or codebase, the visual output must be grounded in observed project facts.

Before generating HTML:

- State the analysis target in one sentence: what part of the project is being explained?
- State assumptions explicitly. If the scope, audience, entrypoint, or source of truth is unclear, ask instead of guessing.
- Inspect only the files needed to answer the request. Do not perform a broad codebase audit unless the user asked for one.
- Prefer primary evidence from the repo: source files, config files, tests, docs, package manifests, and diffs.
- Distinguish facts from inferences. Do not present guessed architecture, intent, ownership, or runtime behavior as fact.
- Trace actual relationships before drawing them: imports, function calls, routes, data flow, configuration references, or documented contracts.
- If a diagram includes uncertain or inferred relationships, label them as inferred or tentative.
- Do not fix, refactor, or clean up project code while analyzing for a visual explanation unless explicitly asked.
...
- Blueprint (technical drawing feel, subtle grid background, deep slate/blue palette, monospace labels, precise borders) — use `./templates/architecture.html` as the closest included structural reference
...
**AI-generated illustrations (optional).** If `surf` is available, you can generate images via Gemini and embed them in the page for creative, illustrative, explanatory, educational, or decorative purposes. Check availability with `which surf`. If available:
...
**Output location:** Use `~/.pi/visual-explainer/` as the persistent workspace for this skill. Generated HTML artifacts go directly in that directory. If other skill-related state is needed, create subdirectories under it (for example `~/.pi/visual-explainer/cache/` or `~/.pi/visual-explainer/assets/`). Create the workspace first if needed (`mkdir -p ~/.pi/visual-explainer`). Use a descriptive filename based on content: `modem-architecture.html`, `pipeline-flow.html`, `schema-overview.html`. For file tools that do not expand `~`, resolve the path to an absolute path before writing.

**Open in browser:**
- macOS: `open ~/.pi/visual-explainer/filename.html`
- Linux: `xdg-open ~/.pi/visual-explainer/filename.html`

**Tell the user** the file path so they can re-open or share it. When the request arrives through the Pi Telegram bridge and the HTML file is the requested artifact, attach the generated file with `telegram_attach` if that tool is available; mentioning the local path is not enough for Telegram delivery.

## Diagram Types

### Architecture / System Diagrams
For existing systems, derive architecture from observed evidence: imports, route registration, service boundaries, config, tests, and docs. Avoid drawing idealized architecture unless the user explicitly asks for a proposed design.
...
When visualizing an implementation plan against an existing project, first compare the plan to the actual codebase. Identify which files, APIs, or modules the plan would affect. Do not invent integration points that are not present in the repo.
...
**Visual richness:** Check `which surf` at the start. If surf-cli is available, consider generating images (title slide background, full-bleed background, optional content illustrations) before writing HTML when they add explanatory value — see the Proactive Imagery section in `slide-patterns.md` for the workflow. Also use SVG decorative accents, per-slide background gradients, inline sparklines, and small Mermaid diagrams. Visual-first, text-second.
...
- **Evidence traceability**: For project-based diagrams, every major component, relationship, and claim should correspond to inspected files, docs, diffs, or commands. Mark uncertain items as inferred.
