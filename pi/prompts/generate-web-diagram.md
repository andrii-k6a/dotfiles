...
argument-hint: "<topic>"
---
Use the `visual-explainer` skill. Read its `SKILL.md` and referenced templates before generating. Then generate an HTML diagram for: $@
...
If `surf` CLI is available (`which surf`), consider generating an AI illustration via `surf gemini --generate-image` when an image would genuinely enhance the page — a hero banner, conceptual illustration, or educational diagram that Mermaid can't express. Match the image style to the page's palette. Embed as base64 data URI. See the "Generated Images" section in `references/css-patterns.md` from the loaded skill for container styles. Skip images when the topic is purely structural or data-driven.
